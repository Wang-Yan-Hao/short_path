...
def main(obstacles, r, s_node, e_node):
    ...
    for i in range(0, obstacle_num):
        for j in range(i + 1, obstacle_num):
            ...
            theta2 = math.atan(
                (obstacle[j][1] - obstacle[i][1]) / (obstacle[j][0] - obstacle[i][0])
            )  # 與水平線夾角
            # 外公切線
            ...
            # 四個可能的切點
            point1 = (
                obstacle[i][0] + radius[i] * math.cos(theta3),
                obstacle[i][1] + radius[i] * math.sin(theta3),
            )
            point2 = (
                obstacle[j][0] + radius[j] * math.cos(theta3),
                obstacle[j][1] + radius[j] * math.sin(theta3),
            )
            point3 = (
                obstacle[i][0] + radius[i] * math.cos(theta4),
                obstacle[i][1] + radius[i] * math.sin(theta4),
            )
            point4 = (
                obstacle[j][0] + radius[j] * math.cos(theta4),
            ...
            if dis == radius[i] + radius[j]:
                point0 = (
                    obstacle[i][0] + radius[i] * math.cos(theta2),
                ...
                # 四個可能的切點
                point5 = (
                    obstacle[i][0] + radius[i] * math.cos(theta5),
                    obstacle[i][1] + radius[i] * math.sin(theta5),
                )
                point6 = (
                    obstacle[j][0] + radius[j] * math.cos(theta5 - math.pi),
                    obstacle[j][1] + radius[j] * math.sin(theta5 - math.pi),
                )
                point7 = (
                    obstacle[i][0] + radius[i] * math.cos(theta6),
                    obstacle[i][1] + radius[i] * math.sin(theta6),
                )
                point8 = (
                    obstacle[j][0] + radius[j] * math.cos(theta6 - math.pi),
